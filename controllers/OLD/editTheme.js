    ...
    .post(function (req, res) {
        ...
        form.parse(req, function (err, fields, files) {
            ...
            if (files.filetoupload != undefined) {
                    ...
                    if (files.filetoupload.size < 1000000){
                        ...
                        fs.rename(oldpath, newpath,  function (err) {
                            if (err){
                                ...
                                // Add Info to DB
                                MongoDB.editTheme(title, new_title, description, filepath).then(function (result) {
                                    if (!result){
                                        sess.message = "An error occured";
                                        res.send({permission: sess.admin, message: sess.message});
                                    } else {
                                        sess.message = 'Theme Edited';
                                        res.send({permission: sess.admin, message: sess.message});
                                    }
                                });
                            }
                ...
                // Add Info to DB
                MongoDB.editTheme(title, new_title, description).then(function (result) {
                    if (!result){
                        sess.message = "An error occured";
                        res.send({permission: sess.admin, message: sess.message});
                    } else {
